...
(function () {
  ...
  function exportExcel() {
    ...
    const licHead = ["رقم الترخيص", "رقم العميل", "المنطقة", "القطعة", "رقم القسيمة", "مساحة القسائم (م²)", "تاريخ التسليم", "رقم طلب الموافقة",
    ...
    EXP.forEach((l) => {
      ...
      groupByClient(l).forEach((g) => licRows.push([l.license, g.client, l.area, blocksJoin(g.plots), plotsJoin(g.plots), sizeSum(g.plots), delivJoin(g.plots), approvalText(l),
    ...
    const licSheet = rtl(XLSX.utils.aoa_to_sheet([licHead, ...licRows]), [12, 11, 22, 10, 14, 15, 15, 18, 30, 30, 50, 14, 12, 13, 13, 13, 34, 34, 50, 50]);
